...
(with-eval-after-load 'lsp-bridge
  (setq lsp-bridge-python-command
        (expand-file-name "~/.asdf/shims/python3")))
...
(use-package tramp
  :straight t
  :init
  ;; 强制从 GNU ELPA 升级（straight 会自动处理）
  (setq straight-built-in-prefer-elpa t))
...
(use-package tramp-rpc
  ...
  :config
  ;; 强制使用 GitHub Release 的预编译二进制，而不是尝试从源码编译
...
(use-package tramp-rpc
  ...
  (tramp-rpc
   ...
   :files ("lisp/*.el"))
  :config (setq tramp-rpc-deploy-prefer-build t))
