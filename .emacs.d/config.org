...
(when *sys/mac*
  (setq plantuml-jar-path "/opt/homebrew/Cellar/plantuml/1.2024.3/libexec/plantuml.jar")
  (setq plantuml-exec-mode 'jar)
...
(use-package emacs-solo-exec-path-from-shell
  ...
  (defun emacs-solo/set-exec-path-from-shell-PATH ()
    ...
    (let ((path-from-shell
      ...
      (message "set path >>> %s" path-from-shell)
  ...
  (defun emacs-solo/fix-asdf-path ()
    ...
    (message ">>> emacs-solo: ASDF PATH fixed"))

  (defun emacs-solo/fix-cargo-path ()
    "Ensure Cargo bin directory (~/.cargo/bin) is first in PATH and exec-path."
    (interactive)
    (let* ((cargo-bin (expand-file-name "~/.cargo/bin"))  ;; 解析 Cargo bin 目录
           (new-paths (list cargo-bin)))

      ;; 清理 exec-path 和 PATH，去掉旧的 Cargo 相关路径
      (setq exec-path (seq-remove (lambda (p) (string-match-p "/\\.cargo/bin" p)) exec-path))
      (setenv "PATH" (string-join (seq-remove (lambda (p) (string-match-p "/\\.cargo/bin" p))
                                              (split-string (getenv "PATH") ":"))
                                  ":"))

      ;; 将 ~/.cargo/bin 目录加入 exec-path 和 PATH
      (dolist (p (reverse new-paths))  ;; 反转顺序，保证优先级
        (unless (member p exec-path) (push p exec-path))  ;; 更新 exec-path
        (unless (member p (split-string (getenv "PATH") ":"))
          (setenv "PATH" (concat p ":" (getenv "PATH"))))))

    (message ">>> emacs-solo: Cargo PATH fixed"))
  ...
  (add-hook 'after-init-hook #'emacs-solo/fix-cargo-path)
...
(use-package projectile
  ...
  :config
  (projectile-mode +1)
  (setq projectile-project-search-path '("~/githubs/")
