...
;(minibuffer-depth-indicate-mode 1)   ; 显示递归层数
;(setq max-mini-window-height 0.5)    ; 防止错误消息刷屏
;(set-face-attribute 'minibuffer-prompt nil :weight 'bold) ; 提示文字更醒目
...
;; 【改动】将 Sarasa Mono SC 放在英文和中文列表的最前面
(setq my/ef (available-font '("Sarasa Mono SC"       ; 新增等宽
...
(setq my/cf (available-font '("Sarasa Mono SC"       ; 新增新增等宽
                               "等距更纱黑体 SC"      ; 备选中文名称
                               "LXGW WenKai Mono"
                               "Microsoft YaHei Mono")))

(defun load-font-setup ()
  "Set up English and Chinese fonts for the current frame."
  (when (display-graphic-p)
    (let ((english-font (or my/ef "Monaco"))
          (chinese-font (or my/cf "PingFang SC"))
          (font-size 180))
      ;; 默认字体设为英文字体（如果 Sarasa 可用，它就是英文首选）
      (set-face-attribute 'default nil :family english-font :height font-size)
      ;; 为中文字符集指定中文字体（如果 Sarasa 可用，它也作为中文首选）
...
(use-package lsp-bridge
  ...
  ;;(lsp-bridge-python-command (expand-file-name "~/.asdf/shims/python3"))
...
;  :after tramp
;  :straight (tramp-rpc :type git :host github :repo "ArthurHeymans/emacs-tramp-rpc" :files ("lisp/*.el"))
;  :config
;  ;; 强制使用 GitHub Release 的预编译二进制文件，避免编译失败
;  (setq tramp-rpc-deploy-git-build-policy 'release))

(use-package inheritenv :straight (:type git :host github :repo "purcell/inheritenv"))
(use-package eat
 :straight (:type git
   :host codeberg
   :repo "akib/emacs-eat"
...
(use-package claude-code
 ...
 :straight (:type git :host github :repo "stevemolitor/claude-code.el" 
            :branch "main" :depth 1 :files ("*.el" (:exclude "images/*")))
 :config
 ...
 ;; kimi 接口（密钥从环境变量 KIMI_API_KEY 读取；取消注释即可使用）
 ;; (progn
   ;;   (setenv "ANTHROPIC_BASE_URL" "https://api.moonshot.cn/anthropic/")
   ;;   (setenv "ANTHROPIC_AUTH_TOKEN" (getenv "KIMI_API_KEY"))
   ;;   (setq claude-code-program "/usr/local/bin/claude")
   ;;   )

  ;; DeepSeek-V4 pro —— 密钥从环境变量 DEEPSEEK_API_KEY 读取
  (progn
