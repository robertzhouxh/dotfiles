...
;; ---- 树语法自动安装与管理 ----
;; 注意：首次运行需通过 M-x treesit-auto-install-all 安装所有语法模块
;; 重新安装（升级）语法需先删除对应 .dylib/.so 文件或 tree-sitter 目录
(use-package treesit-auto
  :demand t
  :init
  (setq treesit-font-lock-level 4)       ; 高亮力度，与 VSCode 一致
  :config
  (setq treesit-auto-install 'prompt)
  (global-treesit-auto-mode))

;; ---- 查看当前点使用的 tree-sitter 语法 ----
(defun treesit-show-parser-used-at-point ()
  "显示当前位置使用的 tree-sitter 解析器。"
  (interactive)
  (if (and (fboundp 'treesit-available-p)
           (treesit-available-p))
      (message "%s" (treesit-language-at (point)))
    (message "tree-sitter 不可用")))

;; ---- 将传统 mode 映射到 tree-sitter 版 mode ----
(setq major-mode-remap-alist
      '((c-mode          . c-ts-mode)
        ...
        ;; (markdown-mode   . markdown-ts-mode)  ; 暂时注释
        ))

;; ---- 为特定模式手动添加 tree-sitter 解析器 ----
;; 以下模式不在 treesit-auto 默认覆盖范围或需要额外创建解析器
(dolist (mode-hook '((markdown-mode-hook  . markdown)
                     (zig-mode-hook       . zig)
                     ...
                     (ruby-mode-hook      . ruby)))
  (add-hook (car mode-hook)
            (lambda () (treesit-parser-create (cdr mode-hook)))))
#+END_SRC
* Programming
...
;; ============================================================
;; Evil-Mode & Key-Bindings (Unified with general.el)
;; ============================================================
...
;; ---- general.el 统一管理全局与 Evil 状态快捷键 ----
(use-package general
  :straight t
  :after evil
  :config
  ;; Evil normal/visual/motion 下的绑定（覆盖默认）
  (general-define-key
   :states '(normal visual motion)
   :keymaps 'override
   ;; 用 swiper 替换 vim 的 / 搜索
   "/"   'swiper
   "?"   'swiper-backward

   ;; LSP 功能键（只在 normal/visual/motion 下生效）
   "C-]"   'lsp-bridge-find-def
   "C-t"   'lsp-bridge-find-def-return
   "M-]"   'lsp-bridge-find-impl
