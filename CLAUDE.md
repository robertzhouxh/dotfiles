...
个人 dotfiles，管理 macOS/Ubuntu 双平台的 Shell、编辑器、Git、终端等配置。主要编辑器是 Emacs，配置采用模块化 `.el` 文件体系（2025 年从 Org-mode 文学编程重构为模块化架构）。
...
- **运行时目录**: `var/` — `custom.el`（Customize 自动生成的配置）、`packages/elpa/`（包缓存）
- **Emacs Server**: `init.el` 结尾自动启动 `(server-start)`，命令行用 `emacsclient -t` / `emacsclient -c` 连接
...
| `lisp/init-elpa.el` | package-archives (MELPA)、bootstrap |
